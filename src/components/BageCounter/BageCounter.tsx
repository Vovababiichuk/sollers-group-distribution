    ...
    <div>
      ...
      <ScrollTrigger onEnter={() => setCounterOn(true)} onExit={() => setCounterOn(false)}>
        <div className="flex items-center">
          <div className="w-ful max-w-max p-3 text-primary rounded text-center">
            <h3 className="font-bold text-[36px] block">
              {counterOn && <CountUp start={0} end={200} duration={2} delay={0} />}+
            </h3>
            <span className="bg-[#fbf500] px-2 py-1 rounded-lg text-[20px] text-black font-medium">Happy Clients</span>
          </div>
          <div className="w-ful max-w-max p-3 text-primary rounded text-center">
            <h3 className="font-bold text-[36px]">
              {counterOn && <CountUp start={0} end={30} duration={2} delay={0} />}%
            </h3>
            <span className="bg-[#fbf500] px-2 py-1 rounded-lg text-[20px] text-black font-medium">Economy Saving</span>
          </div>
          <div className="w-ful max-w-max p-3 text-primary rounded text-center">
            <h3 className="font-bold text-[36px]">
              {counterOn && <CountUp start={0} end={100} duration={2} delay={0} />}%
            </h3>
            <span className="text-[20px] text-black bg-[#fbf500] px-2 py-1 rounded-lg font-medium">
              Dependability
            </span>
          </div>
        </div>
